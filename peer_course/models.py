...
class CourseMember(models.Model):
    ...
    def late_units_used(self):
        return (
            self.submissions_authored.aggregate(models.Sum("late_units_used")).get(
                "late_units_used__sum", 0.0
            )
            or 0.0
        )

    def late_units_available(self):
        return self.course.total_late_units - self.late_units_used()

    def late_available_per_sub(self, assignment):
        return min(self.late_units_available(), assignment.max_late_units)

    def can_compose_submission(self, submission=None, assignment=None):
        # TODO: add grace period here
        if submission is None:
            used = 0
        else:
            assignment = submission.assignment
            used = submission.late_units_used
        if not assignment.submission_required:
            return False
        deadline = assignment.grace_deadline()
        if deadline > timezone.now():
            return True
        hard_deadline = deadline + datetime.timedelta(
            days=self.late_available_per_sub(assignment)
        )
        if hard_deadline > timezone.now():
            return True
        else:
            return False

    def get_pending_reviews(self):
        answered = apps.get_model(
            "peer_review", "ReviewContent"
        )._default_manager.filter(review_assignment=models.OuterRef("pk"))
        return self.reviewassignment_set.annotate(
            finished=models.Exists(answered)
        ).filter(finished=False)

    def get_late_reviews(self):
        return [ra for ra in self.get_pending_reviews() if ra.deadline_passed()]

    def get_pending_apeals(self):
        return self.appeal_set.exclude(
            status__in=[status_choices.CLOSED, status_choices.RESOLVED]
        )
