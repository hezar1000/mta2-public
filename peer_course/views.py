...
from peer_grade.base import AppealBase
...
class CourseViews:
    ...
    def user_view(request, cid, uid):
        ...
        assignments_all = (
            Assignment._default_manager.filter(
        ...
        review_list = ReviewBase.get_my_reviews(user, course, "pending")
        evaluation_list = EvaluationBase.get_my_evaluations(user, course, "pending")
        appeal_list = AppealBase.find_pending_by_assignee(user, course.id)

        render_dict["submissions"] = EvaluationBase.get_my_grades(user, course)
